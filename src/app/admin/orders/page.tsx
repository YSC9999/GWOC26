      ...
      {orders.length === 0 ? (
        ...
        <div className="space-y-4">
          {orders.map((order) => {
            ...
            return (
              <div key={order._id} className="bg-white rounded-2xl border border-gray-100 p-6 shadow-sm hover:shadow-md transition-shadow group">
                ...
                >
                  <div className="flex-1">
                    ...
                    <div className="text-sm text-soil/60 ml-10">
                      {order.userId && typeof order.userId !== 'string' ? order.userId.name : 'Guest'}
                      <span className="mx-2">•</span>
                      {order.items.length} Items
                    </div>
                    <div className="text-xs text-soil/40 mt-1 ml-10">Placed on {new Date(order.createdAt).toLocaleString()}</div>
                  </div>
                  ...
                  <div className="flex items-center gap-6" onClick={(e) => e.stopPropagation()}>
                    ...
                    <div className="text-right min-w-[100px]">
                      ...
                      <div className={`text-xs font-bold uppercase tracking-wider ${order.paymentStatus === 'paid' ? 'text-green-600' : 'text-amber-500'}`}>
                        {order.paymentStatus}
                      </div>
                    </div>
                  </div>

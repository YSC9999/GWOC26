    ...
    <div className="min-h-screen bg-gradient-to-br from-amber-50 via-orange-50 to-clay/10 p-2 md:p-6 font-sans text-soil" onClick={() => setTargetMenuOpen(false)}>
      ...
      <div className="flex flex-col gap-6 mb-8 max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-[1fr_auto_auto] gap-3 md:gap-4 bg-white/40 backdrop-blur-md border border-white/50 p-3 md:p-4 rounded-xl shadow-sm w-full relative z-[60]">
          ...
          <button onClick={() => fetchAnalytics()} className="bg-soil text-white px-6 py-2.5 rounded-lg hover:bg-clay transition-colors shadow-lg shadow-soil/10 h-full flex justify-center items-center gap-2 font-bold uppercase tracking-wider text-xs md:text-sm">
            <Filter size={16} /> Filter
          </button>
          {(startDate || endDate) && (
            <button onClick={handleClearFilter} className="bg-red-50 text-red-600 border border-red-100 px-6 py-2.5 rounded-lg hover:bg-red-100 transition-colors h-full flex justify-center items-center gap-2 font-bold uppercase tracking-wider text-xs md:text-sm">
              <XIcon size={16} /> Clear
            </button>
          )}
        </div>
              ...
              <div className="space-y-6">
                {data.topProducts.slice(0, 3).map((product: any, i: number) => {
                  const percentage = data.summary.totalRevenue > 0
                    ? Math.min(100, (product.revenue / data.summary.totalRevenue) * 100)
                    : 0;
                  return (
                    <div key={i}>
                      <div className="flex justify-between text-sm mb-1">
                        <span className="font-bold text-soil truncate pr-2">{product.name}</span>
                        <span className="font-bold text-clay shrink-0">₹{product.revenue.toLocaleString()}</span>
                      </div>
          ...
          <div className="bg-white/40 backdrop-blur-md p-4 rounded-3xl shadow-sm border border-white/50 space-y-4">
            ...
            <div className="grid grid-cols-1 min-[340px]:grid-cols-2 gap-2 md:gap-4">
              <div className="p-4 bg-red-50/50 border border-red-100 rounded-2xl">
                ...
                <p className="text-xl md:text-2xl font-bold text-red-800 break-all">{data.summary.cancelledOrders || 0}</p>
              ...
              <div className="p-4 bg-red-50/50 border border-red-100 rounded-2xl">
                ...
                <p className="text-xl md:text-2xl font-bold text-red-800 break-all">₹{(data.summary.cancelledRevenue || 0).toLocaleString()}</p>

...
export default function AdminCustomOrderDetailPage({
  ...
  const handleRemoveItem = (index: number) => {
    const reason = prompt(
      "Enter reason for removal (e.g., 'Out of stock', 'Cannot make'):"
    );
    if (!reason) return;
  ...
  const saveOrder = async (newStatus?: string) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setOrder(data);
        alert(
          newStatus === "quoted" ? "Quote sent to user!" : "Changes saved!"
        );
            ...
            <div className="space-y-4">
              ...
              {order.dimensions && (
                <div>
                  ...
                  <span className="font-medium text-soil">
                    {order.dimensions.height} x {order.dimensions.width} x{" "}
              ...
              {order.referenceImages?.length > 0 && (
                <div>
                  <label className="text-xs text-soil/50 uppercase block mb-2">
                    References
                  </label>
                  <div className="grid grid-cols-3 gap-2">
                    {order.referenceImages.map((img, i) => (
                      <a
                        ...
                        className="block aspect-square relative hover:opacity-80 transition-opacity"
                      >
                        <img
                          src={img}
                          alt="ref"
        ...
        <div className="lg:col-span-2 space-y-8">
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
            ...
            <div className="border-t border-gray-100 pt-6">
              <h3 className="text-sm font-bold text-soil mb-4 uppercase">
                Add Item
              </h3>

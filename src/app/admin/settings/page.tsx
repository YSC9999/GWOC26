...
export default function AdminSettings() {
    ...
    return (
        <div className="max-w-4xl mx-auto p-6">
            <h1 className="text-3xl font-bold text-soil mb-8 font-serif">Store Settings</h1>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div className="bg-sand/10 p-6 rounded-xl border border-soil/10">
                            ...
                            <label className={`flex-1 p-4 rounded-xl border-2 cursor-pointer transition-all ${settings.shippingMode === 'weight' ? 'border-clay bg-white shadow-md' : 'border-dashed border-soil/20 hover:bg-white/50'}`}>
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <div className="text-xs text-soil/60 mt-1">Calculates cost based on total weight of items.</div>
                                    </div>
                                </div>
                            </label>
                            <label className={`flex-1 p-4 rounded-xl border-2 cursor-pointer transition-all ${settings.shippingMode === 'pincode' ? 'border-clay bg-white shadow-md' : 'border-dashed border-soil/20 hover:bg-white/50'}`}>
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <div className="text-xs text-soil/60 mt-1">Flat rate or per-pincode (coming soon).</div>
                                    </div>
                                </div>
                            </label>
                        </div>
                        ...
                        {settings.shippingMode === 'pincode' && (
                            <div className="mt-4">
                                <label className="block text-xs font-bold text-soil/70 mb-1">Standard Pincode Rate (₹)</label>
                                <input
                                    type="number"
                    ...
                    <div className="bg-sand/10 p-6 rounded-xl border border-soil/10">
                        ...
                        <p className="text-xs text-soil/60 mb-4">
                            If the cart total exceeds this amount, shipping will be free. Set to 0 to disable.
                        </p>
                        ...
                        {settings.freeShippingThreshold > 0 && (
                            <div className="mt-4 text-sm text-green-600 flex items-center gap-2 bg-green-50 p-2 rounded-lg border border-green-200">
                                <AlertCircle size={16} /> Customers spending over ₹{settings.freeShippingThreshold} get FREE shipping.
                            </div>
                        )}
                    </div>
                </div>
            ...
            <section className="bg-white rounded-2xl p-8 shadow-sm border border-soil/10">
                ...
                <p className="text-sm text-soil/60 mb-6">
                    Define shipping costs based on order weight. The system will calculate the total weight of the cart and apply the matching rate.
                </p>
                    ...
                    <table className="w-full text-left text-sm">
                        ...
                        <tbody className="divide-y divide-soil/5">
                            {rates.length > 0 ? (
                                ...
                                <tr>
                                    <td colSpan={4} className="p-8 text-center text-soil/40 italic">
                ...
                <div className="bg-sand/10 rounded-xl p-6 border border-2 border-dashed border-soil/10">
                    <h3 className="font-bold text-soil mb-4 text-sm uppercase tracking-wide">Add New Rate Rule</h3>
                    <form onSubmit={handleAddRate} className="flex flex-col md:flex-row gap-4 items-end">
                        <div className="flex-1">
                            <label className="block text-xs font-bold text-soil/70 mb-1">Min Weight (kg)</label>
                            <input
                                type="number"
                                step="0.1"
                                required
                        ...
                        <div className="flex-1">
                            <label className="block text-xs font-bold text-soil/70 mb-1">Max Weight (kg)</label>
                            <input
                                type="number"
                                step="0.1"
                                required
                        ...
                        <div className="flex-1">
                            <label className="block text-xs font-bold text-soil/70 mb-1">Cost (₹)</label>
                            <input
                                type="number"
                                required
                                value={newRate.rate}

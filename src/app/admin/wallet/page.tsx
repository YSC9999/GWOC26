...
export default function AdminWalletPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (res.ok) {
        setStatus({
          type: "success",
          msg: data.message || "Funds added successfully!",
        });
        setAmount("");
  ...
  return (
    <div className="p-4 md:p-8 max-w-4xl mx-auto">
      <h1 className="text-2xl sm:text-3xl md:text-4xl font-serif font-bold text-soil mb-8 flex items-center gap-3">
      ...
      <div className="bg-white/80 backdrop-blur-sm rounded-3xl shadow-lg border border-soil/10 overflow-hidden">
        ...
        <div className="flex border-b border-soil/10">
          <button
            ...
            className={`flex-1 p-4 font-bold flex items-center justify-center gap-2 transition-colors ${
              activeTab === "single"
                ? "bg-soil text-white"
                : "text-soil hover:bg-soil/5"
            }`}
          >
  ...
  <div className="p-8">
    ...
    <div className="p-8">
      {activeTab === "single" ? (
        <div className="space-y-6">
          <div>
            <label className="block text-sm font-bold text-soil mb-2">
              Search User
            </label>
            <div className="relative">
              <input
              ...
              />
              <Search
                className="absolute left-3 top-1/2 -translate-y-1/2 text-soil/40"
                size={18}
              />
              {searching && (
                <div className="absolute right-3 top-1/2 -translate-y-1/2">
          ...
          {searchQuery.length > 2 && !selectedUser && (
            <div className="border border-soil/10 rounded-xl overflow-hidden max-h-60 overflow-y-auto bg-white">
              {searchResults.length === 0 && !searching ? (
                <div className="p-4 text-center text-soil/50 text-sm">
                  No users found
                </div>
              ) : (
                searchResults.map((user) => (
                  ...
                  >
                    <div>
                      <div className="font-bold text-soil">{user.name}</div>
                      <div className="text-xs opacity-60 text-soil">
                        {user.email}
                      </div>
                    </div>
                    <div className="text-green-600 font-bold bg-green-50 px-2 py-1 rounded text-xs">
          ...
          {selectedUser && (
            <div className="bg-soil/5 p-4 rounded-xl flex justify-between items-center border border-soil/10">
              <div>
                <span className="text-xs uppercase tracking-wider opacity-60 font-bold text-soil">
                  Selected User
                </span>
                <div className="font-bold text-xl text-soil">
                  {selectedUser.name}
                </div>
                <div className="text-sm opacity-60 text-soil">
                  {selectedUser.email}
                </div>
          ...
          <div className="bg-gradient-to-r from-clay/10 to-orange-100 p-6 rounded-2xl border border-clay/20 flex items-center gap-4">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-soil/70">
                Randomly select users and add funds to their wallet.
              </p>
            </div>
          </div>
      ...
      <div className="mt-8 space-y-6 border-t border-soil/10 pt-8">
        ...
        <button
          disabled={
            loading || (activeTab === "single" && !selectedUser) || !amount
          }
          onClick={handleSubmit}
          className="w-full bg-green-600 text-white py-4 rounded-xl font-bold text-lg hover:bg-green-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
        >
          {loading ? (
            ...
            <>
              ...
              {activeTab === "single"
                ? "Add Funds to User"
                : `Distribute to ${randomCount} Users`}
            </>
          )}
        </button>
        ...
        {status && (
          <motion.div
            initial={{ opacity: 0, y: 10 }}
            animate={{ opacity: 1, y: 0 }}
            className={`p-4 rounded-xl text-center font-bold ${status.type === "success"

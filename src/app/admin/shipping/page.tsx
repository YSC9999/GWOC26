...
export default function AdminShipping() {
  ...
  const [newPincodeRate, setNewPincodeRate] = useState({
    pincode: "",
    rate: "",
    description: "",
  });
      ...
      if (res.ok && data.settings) {
        setSettings({
          ...
          shiprocketReferencePincode:
            data.settings.shiprocketReferencePincode || "110001",
          shiprocketReferencePrice:
            data.settings.shiprocketReferencePrice || 50,
        });
      }
    } catch (err) {
      console.error(err);
  ...
  const handleAddPincodeRate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isBulkPincode) {
        ...
        const codes = bulkPincodesBuffer
          .split(/[\s,]+/)
          .map((s) => s.trim())
          .filter((s) => s.length > 0);
        if (codes.length === 0) {
      ...
      setError("Failed to add pincode rate");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <AdminPageContainer title="Shipping Management">
                ...
                >
                  <div className="flex items-start gap-2">
                    ...
                    <div className="min-w-0 w-full break-words">
                      <div className="font-bold text-soil text-sm sm:text-base leading-tight">
                        Weight Based
                      </div>
          ...
          <section className="bg-amber-50/40 backdrop-blur-md rounded-2xl p-4 md:p-8 shadow-sm border border-white/50">
            ...
            <div className="bg-sand/10 rounded-xl p-6 border-2 border-dashed border-soil/10">
              ...
              <form
                ...
                className="grid grid-cols-1 md:flex md:flex-row gap-4 items-stretch md:items-end"
              >
                <input
                  type="number"
                  step="0.1"
          ...
          <section className="bg-amber-50/40 backdrop-blur-md rounded-2xl p-4 md:p-8 shadow-sm border border-white/50">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
              {[
                {
                  id: "standard",
                  label: "Standard Rate Only",
                  desc: "Flat rate for all pincodes",
                },
                {
                  id: "specific",
                  label: "Specific + Fallback",
                  desc: "Custom rates for specific pincodes",
                },
                {
                  id: "shiprocket_realtime",
                  label: "Shiprocket (Real-time)",
                  desc: "Actual courier rates via API",
                  icon: <Truck size={16} />,
                },
                {
                  id: "shiprocket_reference",
                  label: "Shiprocket (Reference)",
                  desc: "Use rate to specific city for all",
                  icon: <Truck size={16} />,
                },
              ].map((opt) => (
                <button
                ...
                >
                  ...
                  <div>
                    <div className="font-bold text-soil text-sm">
                      {opt.label}
                    </div>
            ...
            <div className="bg-sand/5 rounded-xl p-6 border border-soil/10">
              {settings.pincodeType === "standard" && (
                <div>
                  <label className="block text-sm font-bold text-soil/70 mb-1">
                    Standard Flat Rate (All Pincodes)
                  </label>
                  <input
                    type="number"
                    ...
                    onChange={(e) =>
                      setSettings({
                        ...settings,
                        pincodeRateDefault: parseFloat(e.target.value),
                      })
                    }
              ...
              {settings.pincodeType === "specific" && (
                <div>
                  ...
                  <p className="text-xs text-soil/50">
                    Used when a specific pincode is not found in the table
                    below.
                  </p>
                </div>
              )}
              ...
              {settings.pincodeType === "shiprocket_realtime" && (
                <div className="flex items-center gap-3 text-soil/80">
                  ...
                  <p className="text-sm">
                    Costs will be calculated automatically based on the
                    customer's actual pincode using Shiprocket.
                  </p>
                </div>
              )}
                ...
                <div>
                  <div className="bg-amber-100/50 p-4 rounded-lg border border-amber-200 mb-4">
                    ...
                    <p className="text-xs text-amber-800/80 leading-relaxed">
                      Instead of setting a fixed price, this mode calculates a{" "}
                      <strong>Price Multiplier</strong> based on your "Reference
                      Pincode" and "Desired Price".
                      <br />
                      <br />
                      <strong>Formula:</strong> Multiplier = (Your Price / Cost
                      to Reference Pincode).
                      <br />
                      <br />
                      This multiplier is then applied to the{" "}
                      <strong>actual shipping cost</strong> for every customer
                      order. This allows you to maintain a consistent profit
                      margin regardless of the customer's location.
                    </p>
                  </div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setSettings({
                        ...settings,
                        shiprocketReferencePincode: e.target.value,
                      })
                    }
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setSettings({
                        ...settings,
                        shiprocketReferencePrice: parseFloat(e.target.value),
                      })
                    }
            ...
            {settings.pincodeType === "specific" && (
              <div className="mt-8">
                      ...
                      {pincodeRates.length > 0 ? (
                        pincodeRates.map((rate) => (
                          <tr key={rate._id} className="hover:bg-sand/10">
                            ...
                            <td className="p-4 text-soil/70">
                              {rate.description || "-"}
                            </td>
                            <td className="p-4 font-bold text-clay">
                              ₹{rate.rate}
                            </td>
                            <td className="p-4 text-right">
                              <button
                                onClick={() =>
                                  handleDeletePincodeRate(rate._id)
                                }
                                className="text-red-500 hover:bg-red-50 p-2 rounded-full"
                              >
                                <Trash2 size={16} />
                        ...
                        <tr>
                          ...
                          >
                            No specific rates added. Fallback rate applies to
                            all.
                          </td>
                        </tr>
                      )}
                ...
                <div className="bg-sand/10 rounded-xl p-6 border-2 border-dashed border-soil/10">
                  ...
                  <form
                    onSubmit={handleAddPincodeRate}
                    className="flex flex-col gap-4"
                  >
                    {isBulkPincode ? (
                      <div className="animate-in fade-in slide-in-from-top-2">
                        <label className="block text-xs font-bold text-soil/70 mb-1">
                          Pincodes (Comma, Space or Newline separated)
                        </label>
                        <textarea
                          value={bulkPincodesBuffer}
                          onChange={(e) =>
                            setBulkPincodesBuffer(e.target.value)
                          }
                        ...
                        <p className="text-xs text-soil/50 mt-1">
                          Paste a list of pincodes here. They will all receive
                          the same rate specified below.
                        </p>
                      </div>
                    ) : (
                    ...
                    <div className="grid grid-cols-1 md:flex md:flex-row gap-4 items-stretch md:items-end">
                      <div className="w-full md:flex-1">
                        ...
                        <input
                          type="text"
                          value={newPincodeRate.description}
                          onChange={(e) =>
                            setNewPincodeRate({
                              ...newPincodeRate,
                      ...
                      <button
                        onClick={handleSaveSettings}
                        disabled={savingSettings}
                        className="w-full md:w-auto bg-clay text-white px-6 py-2.5 rounded-xl hover:bg-clay/90 transition-all shadow-lg shadow-clay/20 flex items-center justify-center gap-2 font-medium disabled:opacity-70 disabled:cursor-not-allowed"
                      >
                        {submitting ? (
                          <Loader2 className="animate-spin" size={18} />
                        ) : isBulkPincode ? (
